...
const chatValidationRules = () => {
  return [
    body('content').trim().custom((value, { req }) => {
    ...
    body('isCodeRequest').optional().toBoolean().isBoolean().withMessage('isCodeRequest must be a boolean')
  ];
};

const roomValidationRules = () => {
  return [
    body('name').trim().isLength({ min: 3, max: 50 }).withMessage('Room name must be 3-50 characters').escape(),
    body('documentId').optional().isMongoId().withMessage('Invalid document ID'),
    body('inviteCode').optional().isString().isLength({ min: 6, max: 6 }).withMessage('Invite code must be 6 characters')
  ];
};
